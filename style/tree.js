	...
	Node.prototype = {
		...
		//生成并显示树的DOM
		showDOM: function(treeName, parent, self) {
	...
	Tree.prototype = {
		...
		add: function(data, toData, tvaversal) {
			...
			if (parent) {
				...
				child.parent = parent;
				this._root.showDOM(data, parent, child);
			} else {
	...
	root.add('轮播图1', 'demo', root.traverseBF);
	root.add('轮播图2', 'demo', root.traverseBF);
	root.add('轮播图3', 'demo', root.traverseBF);
	root.add('懒加载', 'demo', root.traverseBF);
	root.add('瀑布流', 'demo', root.traverseBF);
	...
	root.add('Bootstrap', 'CSS', root.traverseBF);